...
class StoriesFragment : Fragment(R.layout.fragment_stories) {
                    ...
                    databaseRef.child("users").child(uid).orderByChild("date").addValueEventListener(object : ValueEventListener{
                        override fun onDataChange(dataSnapShot: DataSnapshot) {
                            ...
                            for (snapShot in dataSnapShot.children){
                                ...
                                Log.d("gameed" , story.date)
                                myStories.add(MyStory(story.uri , simpleDateFormat.parse(story.date) , story.description))

                            }

                            val currentDate = simpleDateFormat.format(Date())
                            Log.d("current" , currentDate)
                            Log.d("story" , lastStory.date)
                            ...
                            Log.d("kkkkkk" , currentDate.substring(0 , 2))
                            Log.d("kkkkkk" , lastStory.date.substring(0 , 2))
                            ...
                            if (hours == 0L && minutes == 0L){
                                ...
                                if (lastStory.date.substring(10, 12).toInt() >= 12){
                                    lastStoryTime = "${(lastStory.date.substring(10, 12).toInt() - 12)}:${lastStory.date.substring(13, 15)} PM"
                            ...
                            myUser = UserStatue(myStories , it.get("name").toString(),lastStoryTime , hastStory , it.get("friends") as ArrayList<String> , it.get("storyUrl").toString() , it.get("myImg").toString())
                                                    ...
                                                    .addValueEventListener(object : ValueEventListener {
                                                        ...
                                                        override fun onDataChange(dataSnapShot: DataSnapshot) {
                                                            ...
                                                            if (hours == 0L  ){
                                                                ...
                                                                lastStoryTime = "$hours:${(minutes% 60)}"
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        addTextStory.setOnClickListener {
            ...
            val intent: Intent = Intent(this.activity, AddTextStory::class.java)
        ...
        addImagedStory.setOnClickListener {
            ...
            addStory.collapseImmediately()
            val intent: Intent = Intent(this.activity, AddImageActivity::class.java)
            startActivity(intent)
        }
